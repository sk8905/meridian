...
import { deals, intel, managers, funds, research, HEDGE_INTEL, HEDGE_FUNDS, LAST_CHECKED, LAST_CHECKED_TIME } from "/credit/js/data.js?v=20260724-6";
import { reportRefresh } from "/v2/js/status.js?v=v2-2";
import { items, cases, restructurings, firmById } from "/legal/js/data.js?v=20260724-2";
import { NEWS, ALERTS, ARTICLES, COMMENTARY, CYCLE, BUBBLE, OUTLOOK } from "/macro/js/content.js?v=20260724-2";
...
function initSavedPanel() {
  ...
  on(document, "click", (e) => { if (!panel.hasAttribute("hidden") && !e.target.closest("#g-saved")) closeAllGlanceMenus(); });
}
...
function wirePalette(idx) {
  ...
  // padding-bottom), so every row clears the keyboard. Pinning it to
  // visualViewport.height instead was fragile on iOS — the keyboard-shrunk panel
  // left results stranded at the bottom with the page showing through.
  ...
  function open() { overlay.classList.add("open"); document.body.classList.add("cmdk-open"); input.value = ""; refresh(); syncClr(); input.focus({ preventScroll: true }); }
  ...
  on(document, "click", (e) => { if (e.target.closest("[data-open-search]")) { e.preventDefault(); open(); } });
